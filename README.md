...
부하·실행계획 결과는 숫자만으로 비교하지 않습니다. 각 기준선에는 commit과 측정 시각, OS·CPU 모델·논리 코어·RAM, 애플리케이션 인스턴스 수·profile·Java/JVM 옵션, MySQL·Docker 버전과 배치, 부하 발생기 버전·배치, 데이터셋 크기, 시나리오, 지연·오류율·실효 RPS, DB·락·Outbox queue age 관찰값을 함께 기록합니다. 비밀값은 기록하지 않습니다.
