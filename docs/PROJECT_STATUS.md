...
- `S11-01` 기능 간 MySQL 회귀, 부하 기준선과 실행계획 검증: `IN_PROGRESS`, issue #8 `OPEN`; 문서 준비 커밋 `9dc903f`, 실제 MySQL 교차 회귀와 전체 30일 분포의 100,000/300,000 `EXPLAIN ANALYZE` profile을 통과함. 현재 Testcontainers plan은 `order_item` scan을 선택했으며, 근거 없이 인덱스를 추가하지 않는다. live k6 기준선은 `local,perf` 앱과 전용 perf DB를 시작한 뒤 실행한다.
...
전용 Compose MySQL을 `up -d --wait`로 시작하고, 새 IntelliJ `local,perf` 구성에서 같은 `PERF_MYSQL_*` 값과 `PERF_SERVER_PORT=18081`로 앱을 시작한 뒤 `powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\scripts\performance\run-local-baseline.ps1 -Seed -RunK6`를 실행한다. 현재 `18080`의 기존 local 앱은 멈추거나 변경하지 않는다.
