...
- `S11-01` 기능 간 MySQL 회귀, 부하 기준선과 실행계획 검증: `READY`, issue #8 `OPEN`; 구현 전 측정 정책과 대상 파일을 Plan에 확정함
...
- S11은 단일 인스턴스에서 Docker k6로 인기 메뉴 GET만 30 RPS·5분 측정하고 p95 500 ms 이하·시스템 오류율 1% 미만을 기준으로 삼는다. 결과에는 OS·CPU·RAM·Docker/Java/MySQL 버전·배치·데이터셋·DB/락/Outbox 관찰값을 함께 기록하며, 실제 사용자 DB의 초기화·seed는 자동화하지 않는다.
- PR 생성과 필수 CI 성공 후에도 자동 병합하지 않는다. 별도 검토와 사용자의 해당 PR 명시적 승인 후에만 병합한다.

## 다음 행동

S11의 실제 MySQL 교차 회귀 테스트를 먼저 작성해 RED를 확인하고, 성능 전용 profile·k6·관찰·환경 기록 도구를 구현한다.
